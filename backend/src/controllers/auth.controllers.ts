...
import { validateEmail, validateLoginInput, validatePassword } from "../validations/user.validations";
...
export const postLoginForgot = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const tokenExpiryDate = moment().add(12, "hours").toDate();
...
export const postLoginReset = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (moment().toDate() > user.passwordResetExpires) {
      return res.status(400).send({
        message:
